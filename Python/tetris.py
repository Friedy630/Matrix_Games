...
def check_fit(shape_matrix: np.ndarray, position: Vec):
    for x in range(shape_matrix.shape[0]):
        for y in range(shape_matrix.shape[1]):
            if shape_matrix[x, y] == 1 and getpixel(position.x + x, position.y + y) != background_color:
                return False
    return True
...
def register_input(key: str):
    ...
    if key in inputs:
        inputs[key] = True
    if key == "exit": running = False   #exit-flag, wird von pygame gesetzt, könnte man aus dem code werfen später
